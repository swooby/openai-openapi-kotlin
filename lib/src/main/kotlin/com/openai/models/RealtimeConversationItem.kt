 ...
 *
 * @param id The unique ID of the item, this can be generated by the client to
 *   help manage server-side context, but is not required because the server
 *   will generate one if not provided.
...
data class RealtimeConversationItem(
    ...
    @Json(name = "type") val type: RealtimeConversationItem.Type? = null,
    ...
    @Json(name = "status") val status: RealtimeConversationItem.Status? = null,

    /* The role of the message sender (`user`, `assistant`, `system`), only  applicable for `message` items.  */
    @Json(name = "role") val role: RealtimeConversationItem.Role? = null,
