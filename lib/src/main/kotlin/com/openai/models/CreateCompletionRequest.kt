 ...
 * @param model
 * @param prompt
 * @param bestOf Generates `best_of` completions server-side and returns the
 *   \"best\" (the one with the highest log probability per token). Results
 *   cannot be streamed. When used with `n`, `best_of` controls the number of
 *   candidate completions and `n` specifies how many to return – `best_of` must
 *   be greater than `n`. **Note:** Because this parameter generates many
 ...
 *   token ID in the GPT tokenizer) to an associated bias value from -100
 *   to 100. You can use this [tokenizer tool](/tokenizer?view=bpe) to convert
 *   text to token IDs. Mathematically, the bias is added to the logits
 *   generated by the model prior to sampling. The exact effect will vary per
 ...
 * @param logprobs Include the log probabilities on the `logprobs` most likely
 *   output tokens, as well the chosen tokens. For example, if `logprobs` is 5,
 ...
 *   generated in the completion. The token count of your prompt plus
 *   `max_tokens` cannot exceed the model's context length.
 *   [Example Python code](https://cookbook.openai.com/examples/how_to_count_tokens_with_tiktoken)
 *   for counting tokens.
 * @param n How many completions to generate for each prompt. **Note:** Because
 *   this parameter generates many completions, it can quickly consume your
 ...
 * @param streamOptions
 * @param suffix The suffix that comes after a completion of inserted text. This
 *   parameter is only supported for `gpt-3.5-turbo-instruct`.
...
data class CreateCompletionRequest(
    @Json(name = "model") val model: CreateCompletionRequestModel,
    @Json(name = "prompt") val prompt: CreateCompletionRequestPrompt?,

    /* Generates `best_of` completions server-side and returns the \"best\" (the one with the highest log probability per token). Results cannot be streamed.  When used with `n`, `best_of` controls the number of candidate completions and `n` specifies how many to return – `best_of` must be greater than `n`.  **Note:** Because this parameter generates many completions, it can quickly consume your token quota. Use carefully and ensure that you have reasonable settings for `max_tokens` and `stop`.  */
    @Json(name = "best_of") val bestOf: kotlin.Int? = 1,

    /* Echo back the prompt in addition to the completion  */
    @Json(name = "echo") val echo: kotlin.Boolean? = false,
    ...
    @Json(name = "logprobs") val logprobs: kotlin.Int? = null,
    ...
    @Json(name = "max_tokens") val maxTokens: kotlin.Int? = 16,
    ...
    @Json(name = "suffix") val suffix: kotlin.String? = null,
