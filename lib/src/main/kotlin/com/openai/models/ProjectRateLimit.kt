/**
 * Please note: This class is auto generated by OpenAPI Generator
 * (https://openapi-generator.tech). Do not edit this file manually.
 */
@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport",
)

package com.openai.models

import com.squareup.moshi.Json
import com.squareup.moshi.JsonClass

/**
 * Represents a project rate limit config.
 *
 * @param `object` The object type, which is always `project.rate_limit`
 * @param id The identifier, which can be referenced in API endpoints.
 * @param model The model this rate limit applies to.
 * @param maxRequestsPer1Minute The maximum requests per minute.
 * @param maxTokensPer1Minute The maximum tokens per minute.
 * @param maxImagesPer1Minute The maximum images per minute. Only present for
 *   relevant models.
 * @param maxAudioMegabytesPer1Minute The maximum audio megabytes per minute.
 *   Only present for relevant models.
 * @param maxRequestsPer1Day The maximum requests per day. Only present for
 *   relevant models.
 * @param batch1DayMaxInputTokens The maximum batch input tokens per day. Only
 *   present for relevant models.
 */
data class ProjectRateLimit(

    /* The object type, which is always `project.rate_limit` */
    @Json(name = "object") val `object`: ProjectRateLimit.`Object`,

    /* The identifier, which can be referenced in API endpoints. */
    @Json(name = "id") val id: kotlin.String,

    /* The model this rate limit applies to. */
    @Json(name = "model") val model: kotlin.String,

    /* The maximum requests per minute. */
    @Json(name = "max_requests_per_1_minute")
    val maxRequestsPer1Minute: kotlin.Int,

    /* The maximum tokens per minute. */
    @Json(name = "max_tokens_per_1_minute") val maxTokensPer1Minute: kotlin.Int,

    /* The maximum images per minute. Only present for relevant models. */
    @Json(name = "max_images_per_1_minute")
    val maxImagesPer1Minute: kotlin.Int? = null,

    /* The maximum audio megabytes per minute. Only present for relevant models. */
    @Json(name = "max_audio_megabytes_per_1_minute")
    val maxAudioMegabytesPer1Minute: kotlin.Int? = null,

    /* The maximum requests per day. Only present for relevant models. */
    @Json(name = "max_requests_per_1_day")
    val maxRequestsPer1Day: kotlin.Int? = null,

    /* The maximum batch input tokens per day. Only present for relevant models. */
    @Json(name = "batch_1_day_max_input_tokens")
    val batch1DayMaxInputTokens: kotlin.Int? = null,
) {

    /**
     * The object type, which is always `project.rate_limit`
     *
     * Values: projectPeriodRate_limit
     */
    @JsonClass(generateAdapter = false)
    enum class `Object`(val value: kotlin.String) {
        @Json(name = "project.rate_limit")
        projectPeriodRate_limit("project.rate_limit")
    }
}
