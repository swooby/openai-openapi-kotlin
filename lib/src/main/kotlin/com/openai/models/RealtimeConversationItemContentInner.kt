...
/**
 * @param type The content type (`input_text`, `input_audio`, `item_reference`,
 *   `text`).
 * @param text The text content, used for `input_text` and `text` content types.
 * @param id ID of a previous conversation item to reference (for
 *   `item_reference` content types in `response.create` events). These can
 *   reference both client and server created items.
 * @param audio Base64-encoded audio bytes, used for `input_audio` content type.
 * @param transcript The transcript of the audio, used for `input_audio` content
 *   type.
 */
data class RealtimeConversationItemContentInner(
    ...
    @Json(name = "audio") val audio: kotlin.String? = null,

    /* The transcript of the audio, used for `input_audio` content type.  */
    @Json(name = "transcript") val transcript: kotlin.String? = null,
) {
