...
/**
 * @param type The truncation strategy to use for the thread. The default is
 *   `auto`. If set to `last_messages`, the thread will be truncated to the n
 *   most recent messages in the thread. When set to `auto`, messages in the
 *   middle of the thread will be dropped to fit the context length of the
 *   model, `max_prompt_tokens`.
 * @param lastMessages The number of most recent messages from the thread when
...
data class CreateRunRequestTruncationStrategy(
    ...
    @Json(name = "type") val type: CreateRunRequestTruncationStrategy.Type,

    /* The number of most recent messages from the thread when constructing the context for the run. */
    @Json(name = "last_messages") val lastMessages: kotlin.Int? = null,
) {
